...
| 属性 | 位宽 | 描述 | 编码示例 |
| :--- | :---: | :--- | :--- |
| **阵营** | 1 bit | `0`: 白方<br>`1`: 黑方 | `0` |
| **种类** | 3 bits | `001`: 王<br>`010`: 后<br>`011`: 象<br>`100`: 马<br>`101`: 车<br>`110`: 兵<br>`111`: (保留) | `110` |
| **位置** | 8 bits | 行 (Row): 4 bits<br>列 (Col): 4 bits<br>| `0000 0000` |

> 综上，只需要 12 bits 就可以完整的表示一个棋子。
> 如 `0 110 000 000` 表示一枚白色的兵在 0 行 0 列。
...
1.  **找到双方“王”的位置**
    *   在棋盘数据结构中遍历，记录白方“王”和黑方“王”的坐标。

2.  **判断是否被直接攻击**
    *   检查对方所有棋子的可攻击范围，判断“王”是否在其中。如果在，则“被将军”。

3.  **判断“王”是否能移动**
    *   检查“王”能否移动到安全位置（不被攻击的格子）。

4.  **判断己方是否有解救办法**
    *   检查己方所有棋子是否能通过移动、吃子、挡住攻击路径等方式解除“被将军”状态。

5.  **判定胜负**
    *   如果“王”被攻击且无法解救，则判定为“被将杀”，游戏结束。
